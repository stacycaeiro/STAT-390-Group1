...
async function submitUserMessage(
	...
	// const token_replacer = new TokenReplacer('sandbox:/mnt/data', '/images')
	...
	(async () => {
		try {
			...
			const eventStream = remoteChain.streamEvents(
				{ messages_str: messages_str, tool_config_by_name: tool_config_by_name }, { version: "v1" }); 
			...
			for await (const event of eventStream) {
				...
				if (eventType === "on_tool_end") {
					...
					const tool_output = typeof tool_end_event.data.output === "string" ? JSON.parse(tool_end_event.data.output) : tool_end_event.data.output;
					
					var toolArgs = {};
					if (tool_start_event !== undefined) {
						toolArgs = typeof tool_start_event.data.input === "string" ? JSON.parse(tool_start_event.data.input) : tool_start_event.data.input;
					}
					...
					streamableUI.append(<Separator className="my-4" />);
					streamableUI.append(render_tool_response_event(tool_end_event));
				} // One tool response at a time
			}
			// streamableUI.update(null);
		} catch (e: any) {
			console.error(e)
			streamableUI.append(<Separator className="my-4" />);
